    ...
    <main className="min-h-screen bg-[#090909] px-6 py-10 text-[#F5E8E8]">
      <div className="mx-auto max-w-7xl">
        <header className="mb-10 max-w-3xl">
          ...
          <p className="mt-4 text-lg leading-8 text-[#F5E8E8]/80">From small freight to regional lanes, Infamous Freight connects quote intake, dispatch, tracking, and proof of delivery in one workflow.</p>
        </header>
          ...
          <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {['Complete quote intake before dispatch', 'Verified driver and carrier handoff', 'Tracking updates through delivery', 'Proof of delivery and support follow-up'].map((item) => (
                <div key={item} className="flex gap-3 rounded-2xl border border-infamous-border bg-infamous-card p-4"><CheckCircle2 className="mt-0.5 flex-shrink-0 text-green-400" size={18} /><p className="text-sm leading-6 text-[#F5E8E8]/80">{item}</p></div>
              ))}
            </div>
          </div>
        </section>
