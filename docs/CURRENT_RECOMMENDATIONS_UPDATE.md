...
The May 14, 2026 04:06 UTC retest confirmed that the canonical web host, apex-to-www redirect, browser-critical `https://www.infamousfreight.com/api/health` path, public quote preflight, and invalid public tracking lookup are working through the Netlify production host. Direct Fly origin checks returned the same public API behavior, and the focused quote-intake and Netlify routing regression tests passed.

The repository now includes exact forced public API proxy routes, an exact forced `/api/health` proxy, and forced `/api/*` and `/socket.io/*` proxy rules ahead of the SPA fallback. The Netlify build configuration intentionally points functions at `netlify/disabled-functions` for normal deploys, and the production readiness deploy command uses the same disabled functions directory so oversized site-level environment variables do not block function upload. Continue using the checks from [`netlify-deploy-checklist.md`](./netlify-deploy-checklist.md) after future production deploys and append the results to the launch evidence log. Do not record secrets or environment variable values in the evidence.
