...
The May 11, 2026 11:07 UTC retest confirmed that the canonical web host and apex-to-www redirect are working. The browser-critical `https://www.infamousfreight.com/api/health` path timed out during this run, so API reachability must be retested before launch. Public Netlify function routes such as `OPTIONS /api/public/quote-requests` and `GET /api/public/shipments/invalid-tracking` still return Netlify 404 responses, and the direct `/.netlify/functions/public-freight` URL also returns 404, which indicates the current production deploy has not exposed the expected function.

The repository now includes exact forced public API function routes, an exact forced `/api/health` proxy, and forced `/api/*` and `/socket.io/*` proxy rules ahead of the SPA fallback. The Netlify build configuration declares `netlify/functions` as the functions directory for normal Git-based deploys, and the production readiness deploy command includes `--functions netlify/functions` for CLI deploys. The public Netlify functions were also moved to standard `.ts` function files to reduce the chance that production deploy packaging skips them, and regression coverage now asserts that the public function entrypoints remain present in the configured deploy directory. The `load-requests` function now declares its own in-code Netlify path metadata, matching the other API functions and reducing routing drift between redirects and function configuration. After the next production deploy, run the checks from [`netlify-deploy-checklist.md`](./netlify-deploy-checklist.md) and append the results to the launch evidence log. Do not record secrets or environment variable values in the evidence.
